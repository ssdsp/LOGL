...
int main()
{
	...
	unsigned int shaderProgram2 = buildShader(vertexShaderSource, frag2ShaderSource);
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(vertices) / 2, (float*)vertices + 9, GL_STATIC_DRAW);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), (void*)0);
	glEnableVertexAttribArray(0);
